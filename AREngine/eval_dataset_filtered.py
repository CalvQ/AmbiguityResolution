...
Uses filtered/simplified scenes (removes generic "Object" entries).

This version applies the same scene preprocessing as dialogue_example.py:
- Parses environment_info into structured objects
- Filters out generic "Object" entries
- Passes clean object list to the engine

Usage:
    python eval_dataset_filtered.py <dataset.json> <output.csv> [--limit N] [--checkpoint-every N] [--resume]
    
Example:
    # Start new evaluation
    python eval_dataset_filtered.py data.json results.csv --checkpoint-every 10
    
    # Resume from checkpoint
    python eval_dataset_filtered.py data.json results.csv --resume
"""
...
def evaluate_with_checkpoints(
    ...
    """
    Evaluate with periodic checkpointing and scene filtering.
    ...
    Args:
        ...
        filter_generic (bool): Filter out generic "Object" entries (default: True)
    """
    # Setup logging
    if log_file:
    ...
    log(f"Scene filtering: {'enabled (removing generic Objects)' if filter_generic else 'disabled'}")
    ...
    if state is None:
        ...
        # Verify filter setting matches
        if state.get('filter_generic', True) != filter_generic:
            log(f"WARNING: filter_generic setting changed from {state.get('filter_generic', True)} to {filter_generic}")
            log("Continuing with original setting from checkpoint")
            filter_generic = state.get('filter_generic', True)
        log("Loading model...")
        engine = AREngine()
        engine.load_model()
        log("Model loaded!")
    ...
    for i in tqdm(
        ...
        # Parse and filter scene (like dialogue_example.py does)
        env_info = (item.get("environment_info") or "").strip()
        scene_objects = parse_environment_info(env_info, filter_generic=filter_generic)
        
        # Set scene on engine with filtered objects
        engine.set_scene(scene_objects)
        
        # Run ambiguity detection (scene already set, pass None)
        try:
            pred, raw = engine.is_prompt_ambiguous(prompt, scene=None)
    ...
    summary = f"""
{'=' * 70}
EVALUATION SUMMARY (Filtered Scenes)
{'=' * 70}

Scene Filtering: {'ENABLED - Generic "Object" entries removed' if filter_generic else 'DISABLED'}
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate AREngine with checkpointing and scene filtering"
    )
    parser.add_argument(
        "json_path",
        type=str,
    ...
    parser.add_argument(
        "--no-filter",
        action="store_true",
        help="Disable scene filtering (keep generic 'Object' entries)"
    )
